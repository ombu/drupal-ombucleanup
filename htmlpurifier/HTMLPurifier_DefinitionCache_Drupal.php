...
class HTMLPurifier_DefinitionCache_Drupal extends HTMLPurifier_DefinitionCache
{
  ...
  function cleanup($config) {
    $results = db_query("SELECT cid FROM {cache} WHERE cid LIKE :cid", array(':cid' => 'htmlpurifier:%'));
    foreach ($results as $record) {
      $key = substr($record->cid, 13); // 13 == strlen('htmlpurifier:')
      if ($this->isOld($key, $config)) {
        cache_clear_all($record->cid, 'cache');
      }
    }
  }
